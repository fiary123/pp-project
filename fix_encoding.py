...
<template>
  <div class="space-y-8 md:space-y-12 pb-16 md:pb-20 px-3 md:px-4 max-w-[1600px] mx-auto text-gray-900 dark:text-white transition-colors duration-300">
    ...
    <!-- 1.5 新增：推荐引擎版块 (New in Phase 2) -->
    <div v-if="authStore.isLoggedIn && engineRecommendations.length > 0" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div class="flex items-center justify-between px-4">
        <div class="flex items-center gap-3">
          <div class="p-2 bg-blue-500/10 text-blue-500 rounded-lg"><Sparkles :size="20" /></div>
          <h3 class="text-xl md:text-2xl font-black italic tracking-tight uppercase">AI 智能优选 <span class="text-xs font-bold text-gray-400 ml-2">基于您的领养画像</span></h3>
        </div>
        <button @click="fetchEngineRecommendations" class="text-xs font-black text-gray-400 hover:text-blue-500 transition-all flex items-center gap-1">刷新优选 <ChevronRight :size="12" /></button>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="rec in engineRecommendations" :key="rec.pet_id" 
          class="bg-blue-50/30 dark:bg-white/2 border border-blue-100 dark:border-white/5 rounded-[2.5rem] p-6 flex gap-5 hover:shadow-xl transition-all group relative overflow-hidden">
          <div class="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
          <div class="w-24 h-24 md:w-32 md:h-32 rounded-3xl overflow-hidden shrink-0 shadow-lg">
            <img :src="rec.image_url" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </div>
          <div class="flex-1 space-y-2 min-w-0">
            <div class="flex justify-between items-start">
              <h4 class="text-xl font-black truncate">{{ rec.pet_name }}</h4>
              <div class="bg-blue-500 text-white px-2 py-1 rounded-lg text-[10px] font-black shadow-md">{{ Math.round(rec.score) }}%</div>
            </div>
            <p class="text-[10px] font-black text-blue-600/60 uppercase tracking-widest">{{ rec.species }}</p>
            <div class="space-y-1.5 pt-1">
              <div v-for="reason in rec.reasons.slice(0, 2)" :key="reason" class="flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-gray-600 dark:text-gray-400">
                <CheckCircle2 :size="12" class="text-green-500" /> {{ reason }}
              </div>
            </div>
            <button @click="selectedPet = pets.find(p => p.id === rec.pet_id) || {id: rec.pet_id, name: rec.pet_name, img: rec.image_url, species: rec.species}" 
              class="pt-2 text-[10px] font-black text-blue-500 hover:underline uppercase tracking-widest">查看优选报告 →</button>
          </div>
        </div>
      </div>
    </div>
...
with open('pet-frontend/src/views/AdoptView.vue', 'w', encoding='utf-8') as f:
    f.write(content)
