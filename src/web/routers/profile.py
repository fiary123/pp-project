...
async def get_preferences(user_id: int, current_user: dict = Depends(get_current_user)):
    """获取用户领养偏好"""
    if current_user["id"] != user_id and current_user["role"] != "admin":
...
async def update_preferences(user_id: int, prefs: UserPreferenceUpdate, current_user: dict = Depends(get_current_user)):
    """更新用户领养偏好"""
    if current_user["id"] != user_id and current_user["role"] != "admin":
